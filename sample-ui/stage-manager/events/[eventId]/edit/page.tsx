	...
	const onSubmit = async (data: EventFormData) => {
		try {
			...
			if (response.ok) {
				...
				// Handle cases where the API did not return JSON (e.g., 404/405 default responses)
				...
				try {
					...
					errorPayload = `HTTP ${response.status} ${response.statusText}`;
				}
				console.error("Failed to update event:", errorPayload);
